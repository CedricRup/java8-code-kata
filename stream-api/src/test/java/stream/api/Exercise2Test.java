...
public class Exercise2Test extends ClassicOnlineStore {
    ...
    public void itemsCustomersWantToBuy() {
        ...
        Stream<Item> itemStream = customerList.stream().flatMap(getItemStream);

        Stream<String> itemNames =  itemStream.map(Item::getName);




        assertTrue(AssertUtil.isLambda(getItemStream));
        List<String> itemList = itemNames.collect(Collectors.toList());
        assertThat(itemList,
